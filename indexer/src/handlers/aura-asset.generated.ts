...
/**
 * Handle CustodyEstablished event from AuraAsset
...
ponder.on('AuraAsset:CustodyEstablished', async ({ event, context }) => {
  ...
  // Insert raw event into event table
  await context.db.insert(custodyEstablished_75e1Events).values({
    id: id,
    token_id: tokenId,
    custodian: custodian,
    amount: amount,
...
/**
 * Handle CustodyReleased event from AuraAsset
...
ponder.on('AuraAsset:CustodyReleased', async ({ event, context }) => {
  ...
  // Insert raw event into event table
  await context.db.insert(custodyReleased_93d2Events).values({
    id: id,
    token_id: tokenId,
    custodian: custodian,
    amount: amount,
