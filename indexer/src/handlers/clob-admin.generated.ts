...
ponder.on(
  'ClobDiamond:CircuitBreakerConfigured',
  async ({ event, context }) => {
    const { marketId, priceChangeThreshold, cooldownPeriod, isEnabled } =
      event.args;
    const id = eventId(event.transaction.hash, event.log.logIndex);
 ...
 * Hash: 0xbae506d4
 */
ponder.on('ClobDiamond:CircuitBreakerReset', async ({ event, context }) => {
  const { marketId, resetAt } = event.args;
  const id = eventId(event.transaction.hash, event.log.logIndex);
 ...
 * Hash: 0x5953204a
 */
ponder.on('ClobDiamond:CircuitBreakerTripped', async ({ event, context }) => {
  const {
    marketId,
    triggerPrice,
    previousPrice,
 ...
 * Hash: 0xaaebcf1b
 */
ponder.on('ClobDiamond:FeeRecipientUpdated', async ({ event, context }) => {
  const { oldRecipient, newRecipient } = event.args;
  const id = eventId(event.transaction.hash, event.log.logIndex);
 ...
 * Hash: 0xb3ef341b
 */
ponder.on('ClobDiamond:FeesUpdated', async ({ event, context }) => {
  const { takerFeeBps, makerFeeBps, lpFeeBps } = event.args;
  const id = eventId(event.transaction.hash, event.log.logIndex);
 ...
 * Hash: 0xa5fea31b
 */
ponder.on('ClobDiamond:GlobalPause', async ({ event, context }) => {
  const { paused } = event.args;
  const id = eventId(event.transaction.hash, event.log.logIndex);
 ...
 * Hash: 0x096cc317
 */
ponder.on('ClobDiamond:MEVProtectionUpdated', async ({ event, context }) => {
  const { minRevealDelay, commitmentThreshold } = event.args;
  const id = eventId(event.transaction.hash, event.log.logIndex);
 ...
 * Hash: 0x613681e6
 */
ponder.on('ClobDiamond:MarketPaused', async ({ event, context }) => {
  const { marketId } = event.args;
  const id = eventId(event.transaction.hash, event.log.logIndex);
 ...
 * Hash: 0xb51d033f
 */
ponder.on('ClobDiamond:MarketUnpaused', async ({ event, context }) => {
  const { marketId } = event.args;
  const id = eventId(event.transaction.hash, event.log.logIndex);
 ...
 * Hash: 0x6675ea6c
 */
ponder.on('ClobDiamond:RateLimitsUpdated', async ({ event, context }) => {
  const { maxOrdersPerBlock, maxVolumePerBlock } = event.args;
  const id = eventId(event.transaction.hash, event.log.logIndex);
