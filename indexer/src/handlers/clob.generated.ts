...
ponder.on('ClobDiamond:CLOBOrderCancelled', async ({ event, context }) => {
  const { orderId, maker, remainingAmount, reason } = event.args;
  const id = eventId(event.transaction.hash, event.log.logIndex);
 ...
 * Hash: 0x2d540948
 */
ponder.on('ClobDiamond:CLOBOrderFilled', async ({ event, context }) => {
  const {
    orderId,
    tradeId,
    fillAmount,
 ...
 * Hash: 0x57e60214
 */
ponder.on('ClobDiamond:CLOBTradeExecuted', async ({ event, context }) => {
  const {
    tradeId,
    takerOrderId,
    makerOrderId,
 ...
 * Hash: 0xb59e4751
 */
ponder.on('ClobDiamond:MarketCreated', async ({ event, context }) => {
  const { marketId, baseToken, baseTokenId, quoteToken } = event.args;
  const id = eventId(event.transaction.hash, event.log.logIndex);
 ...
 * Hash: 0x43fe20c0
 */
ponder.on('ClobDiamond:OrderCreated', async ({ event, context }) => {
  const {
    orderId,
    marketId,
    maker,
 ...
 * Hash: 0xb558d548
 */
ponder.on('ClobDiamond:OrderExpired', async ({ event, context }) => {
  const { orderId, expiredAt } = event.args;
  const id = eventId(event.transaction.hash, event.log.logIndex);
 ...
 * Hash: 0xe764a4f2
 */
ponder.on('ClobDiamond:OrderPlacedWithTokens', async ({ event, context }) => {
  const {
    orderId,
    maker,
    baseToken,
 ...
 * Hash: 0x0e2e2fa3
 */
ponder.on('ClobDiamond:RouterOrderPlaced', async ({ event, context }) => {
  const {
    orderId,
    maker,
    baseToken,
