...
import { nodeSellOrderPlaced } from '../../generated-schema';
import { nodeUpdated } from '../../generated-schema';
import { supportedAssetAdded } from '../../generated-schema';
import { supportedAssetsUpdated } from '../../generated-schema';
import { tokensDepositedToNode } from '../../generated-schema';
import { tokensMintedToNode } from '../../generated-schema';
import { tokensTransferredBetweenNodes } from '../../generated-schema';
import { tokensWithdrawnFromNode } from '../../generated-schema';
import { updateLocation } from '../../generated-schema';
import { updateOwner } from '../../generated-schema';
import { updateStatus } from '../../generated-schema';
...
ponder.on('Diamond:ClobApprovalGranted', async ({ event, context }) => {
  ...
  await context.db.insert(clobApprovalGranted).values({
    id,
    node_hash: nodeHash,
...
ponder.on('Diamond:ClobApprovalRevoked', async ({ event, context }) => {
  ...
  // Insert raw event into event table
  await context.db.insert(clobApprovalRevoked).values({
    id,
    node_hash: nodeHash,
    clob_address: clobAddress,
    block_number: event.block.number,
...
ponder.on('Diamond:NodeCapacityUpdated', async ({ event, context }) => {
  ...
  // Insert raw event into event table
  await context.db.insert(nodeCapacityUpdated).values({
    id,
    node_hash: nodeHash,
...
ponder.on('Diamond:NodeDeactivated', async ({ event, context }) => {
  ...
  // Insert raw event into event table
  await context.db.insert(nodeDeactivated).values({
    id,
    node_hash: nodeHash,
    block_number: event.block.number,
    block_timestamp: BigInt(event.block.timestamp),
...
ponder.on('Diamond:NodeRegistered', async ({ event, context }) => {
  ...
  // Insert raw event into event table
  await context.db.insert(nodeRegistered).values({
    id,
    node_hash: nodeHash,
    owner: owner,
    node_type: nodeType,
...
ponder.on('Diamond:NodeSellOrderPlaced', async ({ event, context }) => {
  ...
  // Insert raw event into event table
  await context.db.insert(nodeSellOrderPlaced).values({
    id,
    node_hash: nodeHash,
    token_id: tokenId,
...
ponder.on('Diamond:NodeUpdated', async ({ event, context }) => {
  ...
  // Insert raw event into event table
  await context.db.insert(nodeUpdated).values({
    id,
    node_hash: nodeHash,
    node_type: nodeType,
    capacity: capacity,
...
ponder.on('Diamond:SupportedAssetAdded', async ({ event, context }) => {
  ...
  // Insert raw event into event table
  await context.db.insert(supportedAssetAdded).values({
    id,
    node_hash: nodeHash,
    token: token,
    token_id: tokenId,
...
ponder.on('Diamond:SupportedAssetsUpdated', async ({ event, context }) => {
  ...
  // Insert raw event into event table
  await context.db.insert(supportedAssetsUpdated).values({
    id,
    node_hash: nodeHash,
    count: count,
    block_number: event.block.number,
...
ponder.on('Diamond:TokensDepositedToNode', async ({ event, context }) => {
  ...
  // Insert raw event into event table
  await context.db.insert(tokensDepositedToNode).values({
    id,
    node_hash: nodeHash,
    token_id: tokenId,
    amount: amount,
...
ponder.on('Diamond:TokensMintedToNode', async ({ event, context }) => {
  ...
  // Insert raw event into event table
  await context.db.insert(tokensMintedToNode).values({
    id,
    node_hash: nodeHash,
    token_id: tokenId,
    amount: amount,
...
ponder.on(
  ...
  async ({ event, context }) => {
    ...
    // Insert raw event into event table
    await context.db.insert(tokensTransferredBetweenNodes).values({
      id,
      from_node: fromNode,
      to_node: toNode,
      token_id: tokenId,
...
ponder.on('Diamond:TokensWithdrawnFromNode', async ({ event, context }) => {
  ...
  // Insert raw event into event table
  await context.db.insert(tokensWithdrawnFromNode).values({
    id,
    node_hash: nodeHash,
    token_id: tokenId,
    amount: amount,
...
ponder.on('Diamond:UpdateLocation', async ({ event, context }) => {
  ...
  // Insert raw event into event table
  await context.db.insert(updateLocation).values({
    id,
...
ponder.on('Diamond:UpdateOwner', async ({ event, context }) => {
  ...
  // Insert raw event into event table
  await context.db.insert(updateOwner).values({
    id,
    owner: owner,
    node: node,
    block_number: event.block.number,
...
ponder.on('Diamond:UpdateStatus', async ({ event, context }) => {
  ...
  // Insert raw event into event table
  await context.db.insert(updateStatus).values({
    id,
    status: status,
    node: node,
    block_number: event.block.number,
