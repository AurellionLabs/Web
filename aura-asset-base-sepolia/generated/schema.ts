...
export class AssetAttributeAdded extends Entity {
  constructor(id: Bytes) {
    super();
    this.set('id', Value.fromBytes(id));
  }
  ...
  save(): void {
    let id = this.get('id');
    assert(id != null, 'Cannot save AssetAttributeAdded entity without an ID');
    if (id) {
      assert(
        id.kind == ValueKind.BYTES,
        `Entities of type AssetAttributeAdded must have an ID of type Bytes but the id '${id.displayData()}' is of type ${id.displayKind()}`,
      );
      store.set('AssetAttributeAdded', id.toBytes().toHexString(), this);
    }
  }

  static loadInBlock(id: Bytes): AssetAttributeAdded | null {
    return changetype<AssetAttributeAdded | null>(
      store.get_in_block('AssetAttributeAdded', id.toHexString()),
    );
  }

  static load(id: Bytes): AssetAttributeAdded | null {
    return changetype<AssetAttributeAdded | null>(
      store.get('AssetAttributeAdded', id.toHexString()),
    );
  }
  ...
  set attributeIndex(value: BigInt) {
    this.set('attributeIndex', Value.fromBigInt(value));
  }
...
export class TransferSingle extends Entity {
  ...
  set internal_id(value: BigInt) {
    this.set('internal_id', Value.fromBigInt(value));
  }
