...
export function handleMintedAsset(event: MintedAssetEvent): void {
  ...
  if (event.params.className != "") {
    ...
    if (!supportedClass) {
      ...
      // Try to get index from contract
      let contract = AuraAsset.bind(event.address)
      let classIndexResult = contract.try_nameToSupportedClassIndex(event.params.className)
  ...
  updateTokenStats(event.params.tokenId, event.params.hash.toHexString(), event.block.timestamp)
  
  // Create or update UserBalance
  updateUserBalance(event.params.account, event.params.tokenId, BigInt.fromI32(1), event.params.hash.toHexString(), event.block.timestamp)
}

export function handleAssetAttributeAdded(event: AssetAttributeAddedEvent): void {
  let attributeId = event.params.hash.toHexString() + "-" + event.params.attributeIndex.toString()
...
export function handleTransferSingle(event: TransferSingleEvent): void {
  ...
  if (event.params.from.notEqual(Address.zero())) {
    ...
    let fromBalanceId = event.params.from.toHexString() + "-" + event.params.id.toString()
  ...
  if (event.params.to.notEqual(Address.zero())) {
    // Add to receiver
    let toBalanceId = event.params.to.toHexString() + "-" + event.params.id.toString()
...
export function handleTransferBatch(event: TransferBatchEvent): void {
  ...
  for (let i = 0; i < tokenIds.length; i++) {
    ...
    if (event.params.from.notEqual(Address.zero())) {
      let fromBalanceId = event.params.from.toHexString() + "-" + tokenId.toString()
      let fromBalance = UserBalance.load(fromBalanceId)
      if (fromBalance) {
    ...
    if (event.params.to.notEqual(Address.zero())) {
      let toBalanceId = event.params.to.toHexString() + "-" + tokenId.toString()
      let toBalance = UserBalance.load(toBalanceId)
      if (!toBalance) {
        toBalance = new UserBalance(toBalanceId)
        toBalance.user = event.params.to
...
function updateTokenStats(tokenId: BigInt, assetId: string, timestamp: BigInt): void {
...
function updateUserBalance(user: Address, tokenId: BigInt, amount: BigInt, assetId: string, timestamp: BigInt): void {
