...
export function handleStaked(event: StakedEvent): void {
  ...
  let stakeId = event.params.operationId.toHexString() + "-" + event.params.user.toHexString()
  ...
  updateUserStats(event.params.user, event.params.amount, true, event.block.timestamp)
  updateTokenStats(event.params.token, event.block.timestamp)
}
...
export function handleUnstaked(event: UnstakedEvent): void {
  ...
  // Update UserStats and TokenStats
  updateUserStats(event.params.user, event.params.amount, false, event.block.timestamp)
  updateTokenStats(event.params.token, event.block.timestamp)
}
...
function updateUserStats(user: Address, amount: BigInt, isStaking: boolean, timestamp: BigInt): void {
  ...
  if (isStaking) {
    ...
    userStats.operationsCount = userStats.operationsCount.plus(BigInt.fromI32(1))
  } else {
