...
export function handleJourneyStatusUpdated(event: JourneyStatusUpdatedEvent): void {
  ...
  if (journey) {
    ...
    if (event.params.newStatus == 1) { // InProgress
      journey.journeyStart = event.block.timestamp
    } else if (event.params.newStatus == 2) { // Completed
...
export function handleOrderCreated(event: OrderCreatedEvent): void {
  ...
  if (!orderResult.reverted) {
    ...
    let parcelData = new ParcelData(event.params.orderId.toHexString() + "-order")
...
export function handleOrderSettled(event: OrderSettledEvent): void {
  ...
  if (order) {
    ...
    updateNodeStats(order.nodes, order.price, order.txFee, event.block.timestamp)
  }
}
...
export function handlePackageSignature(event: PackageSignatureEvent): void {
  ...
  if (journey) {
    ...
    } else if (journey.driver && event.params.user.equals(journey.driver as Bytes)) {
...
function updateDriverStats(driver: Address, timestamp: BigInt, completed: boolean): void {
  ...
  if (completed) {
    driverStats.completedJourneys = driverStats.completedJourneys.plus(BigInt.fromI32(1))
  }
...
function updateNodeStats(nodes: Bytes[], price: BigInt, fee: BigInt, timestamp: BigInt): void {
  for (let i = 0; i < nodes.length; i++) {
    ...
    nodeStats.completedOrders = nodeStats.completedOrders.plus(BigInt.fromI32(1))
    nodeStats.totalRevenue = nodeStats.totalRevenue.plus(price)
    nodeStats.totalFeesEarned = nodeStats.totalFeesEarned.plus(fee.div(BigInt.fromI32(nodes.length)))
