    ...
    new Promise(async (resolve, reject) => {
        try {
            ...
            if (!NEXT_PUBLIC_AUSTAKE_ADDRESS)
                throw new Error(`NEXT_PUBLIC_AUSTAKE_ADDRESS is undefined ${NEXT_PUBLIC_AUSTAKE_ADDRESS}`);
...
const getAuraContract = async (): Promise<AuraGoat> =>
    new Promise(async (resolve, reject) => {
        try {
            ...
            const contract = AuraGoat__factory.connect(NEXT_PUBLIC_AURA_ADDRESS, signer);
            resolve(contract);
        } catch (error) {
            console.error('Error fetching contract:', error);
            reject(error);
