                    ...
                    <tbody>
                        {
                            transactions ? transactions.map((tx, index) => (
                                <tr
                                    ...
                                    <td className="py-4 px-4 text-gray-400">{formatDaysLeft(Number(tx.time))}</td>
                                    <td
                                        className={`py-4 px-4 ${tx.eType === 'Unstaked'
                                            ? 'text-red-500'
                                            : tx.eType === 'Staked'
                                                ? 'text-green-500'
                                                : 'text-gray-400'}`}>
                                    ...
                                    <td className="py-4 px-4 text-right">{formatEthereumValue(tx.amount)}</td>
                                    <td className="py-4 px-4 text-right">{formatEthereumValue(tx.amount)}</td>
                                </tr>
                            ))
