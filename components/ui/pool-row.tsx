    ...
    return (
        <tr className={`border-b border-[${colors.neutral[800]}] hover:bg-[${colors.neutral[800]}]/50 transition-colors`}>
            ...
            <td className="py-4 px-4">
                <Link href={`/pools/${operation.id}`} className={`flex items-center gap-2 hover:text-[${colors.primary[500]}]`} onClick={handleClick}>
                    ...
                    <span className="text-green-500 text-sm">{formattedValues.reward}%</span>
                </Link>
            </td>
            <td className="py-4 px-4 text-right">{formattedValues.tokenTvl}</td>
            <td className="py-4 px-4 text-right text-green-500">{formattedValues.reward}%</td>
            <td className="py-4 px-4 text-right">{formattedValues.tokenTvl}</td>
        </tr>
    );
}
