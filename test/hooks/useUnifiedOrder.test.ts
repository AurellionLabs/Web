...
describe('useUnifiedOrder Hook', () => {
  ...
  describe('Initial State', () => {
    it('should return null order when orderId is provided but not found', async () => {
      ...
      const { result } = renderHook(() => useUnifiedOrder('0xnonexistent', false));
    ...
    it('should handle errors gracefully', async () => {
      mockOrderBridgeService.getUnifiedOrder.mockRejectedValue(new Error('Network error'));
  ...
  describe('Order Actions', () => {
    it('should cancel order successfully', async () => {
      ...
      mockOrderBridgeService.getUnifiedOrder.mockResolvedValue(mockOrder);
      mockOrderBridgeService.cancelUnifiedOrder.mockResolvedValue({ success: true });
      ...
      expect(mockOrderBridgeService.cancelUnifiedOrder).toHaveBeenCalledWith('0x1234');
    });
  ...
  describe('Auto Refresh', () => {
    it('should not auto-refresh when disabled', async () => {
      mockOrderBridgeService.getUnifiedOrder.mockResolvedValue(createMockOrder());
    ...
    it('should auto-refresh when enabled', async () => {
      ...
      const { result } = renderHook(() => useUnifiedOrder('0x1234', true, 5000));
...
describe('useUnifiedOrders Hook', () => {
  ...
  describe('User Orders', () => {
    it('should fetch all orders for a user', async () => {
      ...
      const mockOrders = mockOrderIds.map((id, index) => createMockOrder({ id, status: index === 0 ? 'pending_trade' : 'in_transit' }));
      ...
      const { result } = renderHook(() => useUnifiedOrders('0x742d35Cc6634C0532925a3b844Bc9e7595f2bD4c', false));
    ...
    it('should filter active orders', async () => {
      const mockOrders = [
        createMockOrder({ id: '0x1111', status: 'pending_trade', isActive: true }),
        createMockOrder({ id: '0x2222', status: 'settled', isActive: false }),
        createMockOrder({ id: '0x3333', status: 'in_transit', isActive: true }),
      ];
      
      mockOrderBridgeService.getBuyerOrders.mockResolvedValue(mockOrders.map(o => o.id));
  ...
  describe('Order Filtering', () => {
    it('should separate orders by status', async () => {
      const pendingOrders = [createMockOrder({ id: '0x1111', status: 'pending_trade' })];
      const inTransitOrders = [createMockOrder({ id: '0x2222', status: 'in_transit' })];
      const completedOrders = [createMockOrder({ id: '0x3333', status: 'settled' })];
      
      const allOrders = [...pendingOrders, ...inTransitOrders, ...completedOrders];
      
      mockOrderBridgeService.getBuyerOrders.mockResolvedValue(allOrders.map(o => o.id));
...
describe('Estimated Delivery Calculation', () => {
  it('should return undefined for pending orders', () => {
    const pendingOrder = createMockOrder({ status: 'pending_trade', deliveredAt: 0 });
  ...
  it('should return delivery date for delivered orders', () => {
    ...
    const deliveredOrder = createMockOrder({ status: 'delivered', deliveredAt });
  ...
  it('should estimate 24-48 hours for in-transit orders', () => {
    ...
    const hoursDifference = (inTransitOrder.estimatedDelivery!.getTime() - createdAt) / (1000 * 60 * 60);
