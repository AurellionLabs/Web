...
vi.mock('@/chain-constants', () => ({
  ...
  NEXT_PUBLIC_ORDER_BRIDGE_ADDRESS: '0x0000000000000000000000000000000000000001',
...
describe('OrderBridgeService', () => {
  ...
  describe('getStatusDisplayText', () => {
    it('should return correct text for each status', () => {
      expect(service.getStatusDisplayText(UnifiedOrderStatus.NONE)).to.equal('Unknown');
      expect(service.getStatusDisplayText(UnifiedOrderStatus.PENDING_TRADE)).to.equal('Order Placed');
      expect(service.getStatusDisplayText(UnifiedOrderStatus.TRADE_MATCHED)).to.equal('Trade Matched');
      expect(service.getStatusDisplayText(UnifiedOrderStatus.LOGISTICS_CREATED)).to.equal('Preparing Delivery');
      expect(service.getStatusDisplayText(UnifiedOrderStatus.IN_TRANSIT)).to.equal('In Transit');
      expect(service.getStatusDisplayText(UnifiedOrderStatus.DELIVERED)).to.equal('Delivered');
      expect(service.getStatusDisplayText(UnifiedOrderStatus.SETTLED)).to.equal('Settled');
      expect(service.getStatusDisplayText(UnifiedOrderStatus.CANCELLED)).to.equal('Cancelled');
    });
  });
  
  describe('getStatusColor', () => {
    it('should return correct color class for each status', () => {
      expect(service.getStatusColor(UnifiedOrderStatus.NONE)).to.equal('text-gray-500');
      expect(service.getStatusColor(UnifiedOrderStatus.PENDING_TRADE)).to.equal('text-blue-500');
      expect(service.getStatusColor(UnifiedOrderStatus.TRADE_MATCHED)).to.equal('text-purple-500');
      expect(service.getStatusColor(UnifiedOrderStatus.IN_TRANSIT)).to.equal('text-orange-500');
      expect(service.getStatusColor(UnifiedOrderStatus.DELIVERED)).to.equal('text-green-500');
      expect(service.getStatusColor(UnifiedOrderStatus.SETTLED)).to.equal('text-emerald-500');
      expect(service.getStatusColor(UnifiedOrderStatus.CANCELLED)).to.equal('text-red-500');
    });
  });
  ...
  describe('UnifiedOrder Status Mapping', () => {
    it('should have correct enum values', () => {
      ...
      expect(UnifiedOrderStatus.LOGISTICS_CREATED).to.equal('logistics_created');
  ...
  describe('Bounty Calculation', () => {
    it('should calculate bounty correctly', () => {
      ...
      const expectedBounty = (orderPrice * BigInt(bountyPercentage)) / BigInt(10000);
  ...
  describe('Protocol Fee Calculation', () => {
    it('should calculate protocol fee correctly', () => {
      ...
      const expectedFee = (orderPrice * BigInt(protocolFeePercentage)) / BigInt(10000);
