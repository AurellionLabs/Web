...
async function main() {
  ...
  if (GNOSIS_SAFE !== '0x...') {
    ...
    console.log('Ownership remains with deployer (update GNOSIS_SAFE constant to transfer)');
  }
  ...
  console.log(`   npx hardhat verify --network baseSepolia ${orderBridgeAddress} ${CLOB_ADDRESS} ${AUSYS_ADDRESS} ${QUOTE_TOKEN_ADDRESS} ${FEE_RECIPIENT}`);
  console.log(`\n2. Update indexer environment variables if needed`);
  console.log(`3. Test the OrderBridge with: npx hardhat test test/OrderBridge.t.sol`);
}
...
function updatePonderConfig(orderBridgeAddress: string, deploymentBlock: number) {
...
function updateDeploymentsJson(orderBridgeAddress: string, deploymentBlock: number) {
